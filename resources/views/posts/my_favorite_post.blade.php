...
<div id="container">
  ...
  <div class="post">
    <div class="post-1">
      <p>{{ $myFavorite->user->username }}さん</p>
      <p>{{ $myFavorite->event_at->format('Y年n月d日')}}</p>
    </div>
    <div class="post-2">
      <a href="/post/{{$myFavorite->id}}" class="post-title"><p>{{ $myFavorite->title }}</p></a>
    </div>
    <div class="post-3">
      <p class="post-sub-category">{{ $myFavorite->PostSubCategory->sub_category }}</p>
      <div class="post-count">
        <p class="post-count-content">View 0</p>
        <p class="post-count-content">コメント数：{{$myFavorite->post_comment_count}}</p>
        @if(!$myFavorite->isLikedBy(Auth::user()))
        <img src="{{ asset('images/heart_border.png') }}" alt="ハート" class="heart post-count-content like-toggle" data-post-id="{{ $myFavorite->id }}"><p class="post-heart">{{$myFavorite->post_favorite_count}}</p>
        @else
        <img src="{{ asset('images/heart.png') }}" alt="ハート" class="heart post-count-content like-toggle" data-post-id="{{ $myFavorite->id }}"><p class="post-heart">{{$myFavorite->post_favorite_count}}</p>
        @endif
      </div>
    </div>
  </div>
